...
function renderFinancialDashboard() {
  const input = document.querySelector("#financialDashboardMonth"); if (!input) return; input.value = state.financialDashboardMonth; const { start, end } = financialMonthBounds(state.financialDashboardMonth); const monthEntries = state.financialEntries.filter((entry) => { const date = financialEntryDate(entry); return date >= start && date <= end; }); const accountPriority = ["mercado pago", "itau", "santander"]; const accounts = state.financialAccounts.filter((account) => account.active).sort((first, second) => { const firstName = normalize(first.name); const secondName = normalize(second.name); const firstPriority = accountPriority.findIndex((name) => firstName.includes(name)); const secondPriority = accountPriority.findIndex((name) => secondName.includes(name)); const firstRank = firstPriority < 0 ? accountPriority.length : firstPriority; const secondRank = secondPriority < 0 ? accountPriority.length : secondPriority; return firstRank - secondRank || financialSortCollator.compare(first.name, second.name); });
